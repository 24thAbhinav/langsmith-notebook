...
In [5_langgraph.py](file:///Users/abhinav/Documents/Projects/langsmith/langsmith-masterclass/5_langgraph.py), a parallel multi-agent evaluation workflow is built using **LangGraph** to grade essays across three distinct dimensions.

#### Key Architectural & Tracing Concepts

1. **Parallel Graph Execution (Fan-Out → Fan-In)**:
   - `START` splits execution into 3 parallel nodes (`evaluate_language`, `evaluate_analysis`, `evaluate_thought`).
   - After all 3 parallel nodes finish, the state converges into `final_evaluation` to generate summary feedback and compute the average score.

2. **State Reducers (`operator.add`)**:
   - The graph state (`UPSCState`) defines `individual_scores: Annotated[List[int], operator.add]`.
   - `operator.add` acts as a reducer function, allowing parallel branches to safely append their scores (`[4]`, `[5]`, `[4]` → `[4, 5, 4]`) into shared state without race conditions or overwrites.

3. **Structured Output Fix for Ollama (`method="json_mode"`)**:
   - Calling `model.with_structured_output(EvaluationSchema)` defaults to native function calling (`method="function_calling"`). Small models like `gemma2:2b` throw an HTTP 400 error (`gemma2:2b does not support tools`).
   - **Solution**: Pass `method="json_mode"` and include explicit key hints (`"feedback"` and `"score"`) in the prompt:

     ```python
     structured_model = model.with_structured_output(EvaluationSchema, method="json_mode")
     ```

4. **Dual Tracing: Graph Nodes + Decorated Python Functions**:
   - `workflow.invoke({"essay": essay2}, config=run_config)` sets the root trace name to `evaluate_upsc_essay`.
